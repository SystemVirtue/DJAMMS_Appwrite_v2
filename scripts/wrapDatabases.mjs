// Lightweight wrapper to sanitize Databases SDK calls in scripts (ESM)
export default function wrapDatabases(rawDatabases) {
    ...
    get(target, prop) {
      ...
      if (prop === 'getDocument') {
        ...
        return async function (...args) {
          ...
          try { return deepRemovePreferences(res); } catch (e) { return res; }
        };
      }
      if (prop === 'listDocuments' || prop === 'list') {
        ...
        return async function (...args) {
          const res = await orig.apply(target, args);
          try {
            if (res && Array.isArray(res.documents)) res.documents = res.documents.map(d => deepRemovePreferences(d));
            return res;
          } catch (e) { return res; }
        };
      }
