<script lang="ts">
	...
	import { Play, Music, Headphones, Zap, AlertTriangle } from 'lucide-svelte';
	...
	onMount(() => {
		...
		isDemoMode = !projectId || projectId === 'your_project_id_here';
	...
	async function handleGoogleLogin() {
		...
		try {
			...
			if (!projectId || projectId === 'your_project_id_here') {
				// Demo mode - set demo user and redirect
				console.warn('Appwrite not configured, running in demo mode');
				auth.setDemoUser();
				setTimeout(() => {
					goto('/djamms-dashboard');
				}, 1000);
				...
				try {
					await account.createOAuth2Session(
						'google',
						`${window.location.origin}/djamms-dashboard`,
	...
	<div class="relative max-w-4xl mx-auto">
		...
		<div class="relative glass-morphism rounded-3xl p-8 md:p-12 text-center">
			...
			<div class="space-y-4">
				<!-- Demo Mode Warning -->
				{#if isDemoMode}
					<div class="mb-6 p-4 bg-orange-500/10 border border-orange-500/30 rounded-xl text-center">
						<div class="flex items-center justify-center gap-2 mb-2">
							<AlertTriangle class="w-5 h-5 text-orange-400" />
							<span class="text-orange-400 font-semibold">Demo Mode</span>
						</div>
						<p class="text-orange-300 text-sm">
							Appwrite not configured. Running in demo mode without authentication.
						</p>
						<p class="text-orange-400 text-xs mt-1">
							See <code class="bg-orange-500/20 px-2 py-1 rounded">APPWRITE_SETUP.md</code> for configuration instructions.
						</p>
					</div>
				{/if}
				...
				>
					{#if isLoading || $auth.isLoading}
						...
						{isDemoMode ? 'Entering Demo Mode...' : 'Signing in...'}
					{:else}
						{#if isDemoMode}
							<Play class="w-5 h-5" />
							Try Demo Mode
